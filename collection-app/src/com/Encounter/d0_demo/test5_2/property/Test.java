 ...
 * 使用LinkedList集合存储住户信息，每个住户信息用一个自定义类Resident表示，类包含姓名、电话、门牌号、欠费金额，重写tostina方法
 * 1.定XPropertyManagementSystem类实现以下方法,使用Scanner类从控制台输入住户信息，格式如下;请输入住户信息(格式:姓名 电话 门牌
 * 号 欠费金额)，输入0结束:
 * ③使用List的add()方法将住户信息添加到LinkedList集合中,
 * ④)使用List的forEach()方法遍历LinkedList集合，查找指定门牌号的住户信息并打印出来,
 * ⑤)使用List的forEach()方法遍历Linkedist集合，统计所有住户的欠费金额和总人数，并打印出来。
